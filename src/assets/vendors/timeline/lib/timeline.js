...
function mouseoverItemEventCallback(eventId,timelineId) {
  $('.event').removeClass("highlighted");
  $('#event_id_'+eventId).addClass("highlighted");
}

function clickItemEventCallback(eventId, dotDiv,event_ids,timelineId){

  $('.timeline-event-dot-'+timelineId).removeClass("selected");
  $(dotDiv).addClass("selected");
  draw_with_filtered_entries(event_ids,timelineId);
}

function draw_with_filtered_entries(event_ids,timelineId){
  var source='';
  $('#hidden_button').trigger('click', [source, event_ids]);
...
links.Timeline = function(container, options) {
  ...
  this.options = {
    ...
    'CLUSTER_TITLE': "Cluster containing {count} alerts with 1 or more alert having {severity} severity . Click to filter the table entries.",
    'AM': "am",
    'PM': "pm"
  };
...
this.render(undefined,timelineId);
...
links.Timeline.prototype.draw = function(data, options) {
  ...
  // read the data
  var itemExist = this.setData(data);
  if(itemExist == true){
    return true;
   }
   else{
 ...
 *                                                        as default value
 */
links.Timeline.prototype.render = function(options,timelineId) {
  ...
  this.highlightClickedItem(timelineId);
};
 ...
 *                                 a reflow is needed.
 */
links.Timeline.prototype.repaint = function(timelineId) {
  var frameNeedsReflow = this.repaintFrame(timelineId);
  var axisNeedsReflow  = this.repaintAxis(timelineId);
  var groupsNeedsReflow  = this.repaintGroups(timelineId);
  var itemsNeedsReflow = this.repaintItems(timelineId);
  this.repaintCurrentTime(timelineId);
  this.repaintCustomTime(timelineId);
 ...
 *                                 a reflow is needed.
 */
links.Timeline.prototype.repaintFrame = function(timelineId) {
  ...
  if (!dom.content) {
    ...
    if (!params.onMouseDown) {
      params.onMouseDown = function (event) {me.onMouseDown(event,timelineId);};
    ...
    if (!params.onMouseWheel) {
      params.onMouseWheel = function (event) {me.onMouseWheel(event,timelineId);};
  ...
  this.repaintNavigation(timelineId);
 ...
 */
links.Timeline.prototype.repaintAxis = function(timelineId) {
  var needsReflow = false,
    dom = this.dom,
    options = this.options,
    size = this.size,
 ...
 *                                 a reflow is needed.
 */
links.Timeline.prototype.repaintItems = function(timelineId) {
  ...
  if (newImageUrls.length) {
    ...
    timeline.render(undefined,timelineId);
    };
 ...
 */
links.Timeline.prototype.repaintGroups = function(timelineId) {
  var dom = this.dom,
    timeline = this,
    options = this.options,
    size = this.size,
 ...
 */
links.Timeline.prototype.repaintCurrentTime = function(timelineId) {
  ...
  var onTimeout = function() {
    timeline.repaintCurrentTime(timelineId);
  };
 ...
 */
links.Timeline.prototype.repaintCustomTime = function(timelineId) {
  var options = this.options,
    dom = this.dom,
    size = this.size;
 ...
 */
links.Timeline.prototype.repaintNavigation = function (timelineId) {
  var timeline = this,
    options = this.options,
    dom = this.dom,
    frame = dom.frame,
  ...
  if (!navBar) {
    ...
    if (showButtonNew) {
      ...
      var onAdd = function(event) {
        ...
        timeline.render({animate: false},timelineId);
          timeline.selectItem(index);
        }
        else {
          // undo an add
    ...
    if (showNavigation) {
      if (options.zoomable) {
        ...
        var onZoomIn = function(event) {
          ...
          links.Timeline.preventDefault(event);
          links.Timeline.stopPropagation(event);
          timeline.zoom(0.4,undefined,timelineId);
 ...
 */
links.Timeline.prototype.onMouseDown = function(event,timelineId) {
  ...
  if (!params.touchDown) {
    ...
    if (!params.onMouseMove) {
      params.onMouseMove = function (event) {me.onMouseMove(event,timelineId);};
      links.Timeline.addEventListener(document, "mousemove", params.onMouseMove);
    }
    if (!params.onMouseUp) {
      params.onMouseUp = function (event) {me.onMouseUp(event,timelineId);};
 ...
 */
links.Timeline.prototype.onMouseMove = function (event,timelineId) {
  event = event || window.event;
  ...
  if (params.customTime) {
    ...
    if (this.groups.length && !dragging) {
      ...
      if (options.groupsChangeable && item.group !== group) {
        ...
        this.changeItem(index, {'group': this.getGroupName(group)},false,timelineId);
      }
      else {
        this.repaintDeleteButton();
        this.repaintDragAreas();
    ...
    this.render(undefined,timelineId); // TODO: optimize, only redraw the items?
 ...
 */
links.Timeline.prototype.onMouseUp = function (event,timelineId) {
  var params = this.eventParams,
    options = this.options;
 ...
 * @param {Event}  event   The event
 */
links.Timeline.prototype.onMouseWheel = function(event,timelineId) {
  if (!this.options.zoomable)
    return;
  ...
  if (delta) {
    ...
    var zoom = function (timelineId) {
      ...
      timeline.zoom(zoomFactor, zoomAroundDate,timelineId);
    ...
    var scroll = function () {
      // Scroll the timeline
      timeline.move(delta * -0.2,timelineId);
      timeline.trigger("rangechange");
      timeline.trigger("rangechanged");
    };
    ...
    if (event.shiftKey) {
      ...
      zoom(undefined,undefined,timelineId);
    }
  }
 ...
 */
links.Timeline.prototype.zoom = function(zoomFactor, zoomAroundDate,timelineId) {
  return;
 ...
 */
links.Timeline.prototype.move = function(moveFactor,timelineId) {
  // zoom start Date and end Date relative to the zoomAroundDate
  var diff = (this.end.valueOf() - this.start.valueOf());
  ...
  this.applyRange(newStart, newEnd);
  this.render(undefined,timelineId); // TODO: optimize, no need to reflow, only to recalc conversion and repaint
};
...
links.Timeline.prototype.applyRange = function (start, end, zoomAroundDate) {
  ...
  //#changed//
  let start_date = this.options.start.valueOf();
  let end_date = this.options.end.valueOf();
  if (start_date && end_date ) {
    // zoom out 5% such that you have a little white space on the left and right
    let diff = (start_date - end_date);
    start_date = start_date ;
    end_date = end_date ;
  }


  var startValue = start_date; // number
  var endValue = end_date;
 ...
 */
links.Timeline.Item.prototype.createDOM = function (timelineId) {
  // Should be implemented by sub-prototype
};
 ...
 */
links.Timeline.Item.prototype.showDOM = function (container,timelineId) {
  // Should be implemented by sub-prototype
};
 ...
 * of the item
 */
links.Timeline.Item.prototype.updateDOM = function (timelineId) {
  // Should be implemented by sub-prototype
};
 ...
 * @override
 */
links.Timeline.ItemBox.prototype.createDOM = function (timelineId) {
  // background box
 ...
 * @override
 */
links.Timeline.ItemBox.prototype.showDOM = function (container,timelineId) {
  var dom = this.dom;

  if (!dom) {
    dom = this.createDOM(timelineId);
  }
 ...
 * @override
 */
links.Timeline.ItemBox.prototype.updateDOM = function (timelineId) {
  ...
  if (divBox) {
    ...
    divDot.className  = "timeline-event timeline-event-dot timeline-event-dot-"+timelineId+" ui-widget ui-state-default";
    ...
    links.Timeline.addEventListener(divDot, 'mouseover', function() {
      ...
      if(mouseoverItemEventCallback != undefined) mouseoverItemEventCallback(this.event_id,timelineId);
    }.bind(this));
    ...
    links.Timeline.addEventListener(divDot, 'click', function() {
      ...
      if(clickItemEventCallback != undefined) clickItemEventCallback(this.event_id, divDot,this.event_ids,timelineId);
    }.bind(this));
    ...
    links.Timeline.addEventListener(divDot, 'keypress', function(event) {
      ...
      if(keycode == 13 && clickItemEventCallback != undefined) {
        selectedEventId = this.event_id;
        clickItemEventCallback(this.event_id, divDot,this.event_ids,timelineId);
      }
    }.bind(this));
 ...
 * @override
 */
links.Timeline.ItemRange.prototype.createDOM = function (timelineId) {
  // background box
  var divBox = document.createElement("DIV");
  divBox.style.position = "absolute";
 ...
 * @override
 */
links.Timeline.ItemRange.prototype.showDOM = function (container,timelineId) {
  var dom = this.dom;
  if (!dom) {
    dom = this.createDOM(timelineId);
  }
 ...
 * @override
 */
links.Timeline.ItemRange.prototype.updateDOM = function (timelineId) {
  var divBox = this.dom;
  if (divBox) {
    // update contents
    divBox.firstChild.innerHTML = this.content;
 ...
 * @override
 */
links.Timeline.ItemFloatingRange.prototype.createDOM = function (timelineId) {
  // background box
  var divBox = document.createElement("DIV");
  divBox.style.position = "absolute";
 ...
 * @override
 */
links.Timeline.ItemFloatingRange.prototype.showDOM = function (container,timelineId) {
 ...
 * @override
 */
links.Timeline.ItemFloatingRange.prototype.updateDOM = function (timelineId) {
  var divBox = this.dom;
  if (divBox) {
    // update contents
    divBox.firstChild.innerHTML = this.content;
 ...
 * @override
 */
links.Timeline.ItemDot.prototype.createDOM = function (timelineId) {
  // background box
  var divBox = document.createElement("DIV");
  divBox.style.position = "absolute";
 ...
 * @override
 */
links.Timeline.ItemDot.prototype.showDOM = function (container,timelineId) {
  var dom = this.dom;
 ...
 * @override
 */
links.Timeline.ItemDot.prototype.updateDOM = function (timelineId) {
  if (this.dom) {
    var divBox = this.dom;
    var divDot = divBox.dot;
 ...
 * @param {boolean} [preventRender=false]   Do not re-render timeline if true
 */
links.Timeline.prototype.changeItem = function (index, itemData, preventRender,timelineId) {
 ...
 *                                         defaults to false.
 */
links.Timeline.prototype.stackItems = function(animate,timelineId) {
...
links.Timeline.prototype.highlightClickedItem = function(timelineId){
